    ...
    <div className="bg-card rounded-2xl overflow-hidden">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 divide-y sm:divide-y-0 divide-x divide-card-dark">
        {stats.map(({ label, value, sub, valueColor }) => (
          <div key={label} className="px-5 py-5">
            ...
            <p className="text-[11px] text-(--color-tertiary-text) mt-1">{sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="px-6 py-4 border-t border-card-dark">
        <div className="flex items-center gap-4 flex-wrap">
          <span className="text-[11px] text-(--color-tertiary-text) flex-shrink-0">Plan Feasibility</span>
          ...
          <span className="text-[12px] font-semibold text-(--color-brand) flex-shrink-0">78% — Achievable</span>
          <span className="text-[10px] text-(--color-tertiary-text) ml-auto hidden lg:block">
