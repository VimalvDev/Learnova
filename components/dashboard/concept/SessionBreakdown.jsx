  ...
  return (
    <div className="bg-card rounded-2xl overflow-hidden">
      ...
      {sessions.map((s, i) => (
        ...
        >
          <span className="text-[11px] text-secondary-text">{s.date}</span>
          ...
          <span className="text-[11px] text-secondary-text">{s.time}</span>
          <span className="text-[12px] text-[#FBBF24]">{s.mistakes} errors</span>
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg w-fit ${resultStyle[s.result] ?? "text-secondary-text bg-card-dark"}`}>
            {s.result}
          </span>
        </div>
      ))}
