    ...
    <div className="pb-5 border-b border-(--color-card)">
      ...
      <div className="flex items-center gap-1.5 flex-wrap mb-3">
        {crumbs.map((c, i) => (
          <div key={c.label} className="flex items-center gap-1.5">
            {i > 0 && <RiArrowRightSLine className="text-dark-gray text-[13px]" />}
            {c.href ? (
              <Link href={c.href} className="text-[12px] text-tertiary-text hover:text-brand transition-colors">
                {c.label}
              </Link>
            ) : (
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-[12px] text-tertiary-text mb-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-[10px] text-brand bg-(--color-brand)/10 px-2.5 py-1 rounded-lg border border-(--color-brand)/20">
