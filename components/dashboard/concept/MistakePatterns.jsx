const patterns = [
  ...
  },
  {
    ...
    countStyle:"text-(--color-brand) bg-(--color-brand)/10",
  ...
  return (
    <div className="bg-card rounded-2xl p-6">
      <div className="flex items-start justify-between mb-5 flex-wrap gap-2">
        ...
        <span className="text-[11px] text-(--color-tertiary-text) self-end">18 total errors logged</span>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {patterns.map(({ dot, severity, count, countStyle, title, detail, evidence, inset, actions }) => (
          <div key={title} className="bg-card-dark rounded-xl p-5">
            ...
            {/* Detail */}
            <p className="text-[12px] text-(--color-secondary-text) leading-relaxed mb-3">{detail}</p>

            {/* Evidence */}
            <p className="text-[10px] text-(--color-tertiary-text) mb-3">{evidence}</p>

            {/* Clarification inset */}
            {inset && (
              <div className="pl-3 border-l-2 border-(--color-brand)/30 mb-3">
                <p className="text-[11px] text-(--color-secondary-text) leading-relaxed font-mono whitespace-pre-line">
                  {inset}
                </p>
              </div>
            )}
                ...
                <button
                  ...
                  className={`text-[11px] font-medium rounded-lg transition-all ${
                    i === actions.length - 1
                      ...
                      : "px-3 py-1.5 bg-card text-(--color-secondary-text) hover:text-white"
                  }`}
                >
                  {a}
                </button>
