export default function PublicQuizPreview({ topic, onSwitchPrivate }) {
  ...
  return (
    <div className="bg-card rounded-2xl p-6">
      ...
      {/* Q1 visible */}
      <div className="mb-4">
        <p className="text-[11px] font-bold text-brand mb-2">Q1</p>
      ...
      {[2, 3].map((n) => (
        <div key={n} className="mb-3" style={{ filter: "blur(3px)", opacity: 0.4, pointerEvents: "none" }}>
          <p className="text-[11px] font-bold text-brand mb-2">Q{n}</p>
      ...
      <div className="flex items-center gap-3 mt-5 pt-4 border-t border-(--color-card-dark)">
        ...
        <button className="h-9 px-4 bg-card-dark text-[12px] text-secondary-text rounded-xl border border-white/[0.06] hover:border-white/[0.1] hover:text-white transition-all">
          Modify Difficulty ▾
        </button>
      </div>
