...
const GLOW = "250, 110, 67" // your brand orange in RGB

export default function Features() {
  const sectionRef = useRef(null)

  // shared props for every inline big card
  const particleProps = {
    glowColor:        GLOW,
    particleCount:    8,
    enableTilt:       false,
  ...
  return (
    ...
    >
      ...
      {/* ── spotlight that follows the cursor over all .card elements ── */}
      <GlobalSpotlight
      ...
      <div className="grid grid-cols-1 mt-[3em] md:grid-cols-12 gap-[.5em]">
        ...
        <ParticleCard
          {...particleProps}
          className="card card--border-glow md:col-span-7 bg-card rounded-xl overflow-hidden relative flex flex-col min-h-[520px]"
        ...
        <ParticleCard
          {...particleProps}
          className="card card--border-glow md:col-span-5 bg-card rounded-xl overflow-hidden relative flex flex-col min-h-[520px]"
          style={{ "--glow-x":"50%","--glow-y":"50%","--glow-intensity":"0","--glow-radius":"200px" }}
        >
          <div className="absolute top-4 right-4 opacity-10 z-0">
            <TbGridDots className="text-white text-2xl" />
        ...
        {/* ── ROW 2 — FeatureCard uses ParticleCard internally ── */}

        <FeatureCard
          num="03" colSpan="md:col-span-4"
          title="Ask Your Notes"
          desc="AI answers sourced strictly from your documents. Every response cites the source."
          tags={["RAG","Citations","Confidence"]}
          chart={<div className="h-full min-h-[280px]"><SimpleRAGChat /></div>}
          link="#ask-your-notes"
        />
        <FeatureCard
          num="04" colSpan="md:col-span-4"
          ...
          chart={<div className="h-full min-h-[280px]"><MiniHeatmap /></div>}
          link="#adaptive-quiz"
        />
        <FeatureCard
          num="05" colSpan="md:col-span-4" minHeight="min-h-[480px]"
          title="Performance Insights"
          desc="Accuracy, speed, retention, consistency — one radar. No guesswork."
          tags={["Radar","6 Dimensions","Trends"]}
          chart={<div className="h-full min-h-[300px]"><MiniRadar /></div>}
          link="#performance-insights"
        />

        {/* ── ROW 3 — full-width revision scheduler ── */}

        <ParticleCard
          {...particleProps}
          className="card card--border-glow md:col-span-12 bg-card rounded-xl overflow-hidden relative flex flex-col min-h-[360px]"
          style={{ "--glow-x":"50%","--glow-y":"50%","--glow-intensity":"0","--glow-radius":"200px" }}
        >
          <div className="absolute top-4 right-4 opacity-10 z-0">
            <TbGridDots className="text-white text-2xl" />
          ...
          <div className="p-[1.8em] flex flex-col h-full gap-4 relative z-10">
            <div className="flex items-start justify-between">
              <div className="flex-1 pr-4">
                <span className="text-[11px] font-bold uppercase tracking-widest text-brand">06</span>
