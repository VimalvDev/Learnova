  ...
  try {
    ...
    if (!docs?.length) {
      return Response.json(
        { error: "No documents found. Upload study materials first." },
        { status: 400 },
      );
    }
    ...
    const { data: masteryScores } = await supabase
      .from("mastery_scores")
      .select(
        "score, status, concept_id, concepts(id, concept_name, document_id)",
      )
      .eq("user_id", user.id)
      .in(
    ...
    const typeInstructions = types
      .map((t) => {
        if (t === "mcq")
          return "MCQ (multiple choice with 4 options, exactly one correct)";
        if (t === "truefalse")
          return "TRUE_FALSE (statement that is either true or false)";
        if (t === "short")
          return "SHORT_ANSWER (requires 1-3 sentence written answer)";
        if (t === "fill")
          return "FILL_BLANK (sentence with one key term blanked out)";
        return t;
      })
      .join(", ");
...
${docContext}`;
    ...
    try {
      ...
      return Response.json(
        { error: "Failed to parse generated questions. Try again." },
        { status: 500 },
      );
    }
    ...
    if (!Array.isArray(questions) || questions.length === 0) {
      return Response.json(
        { error: "No questions generated. Check your documents." },
        { status: 500 },
      );
    }
    ...
    const { data: savedQuestions } = await supabase
      ...
      .select(
        "id, question_type, question_text, options, correct_answer, explanation, difficulty, question_order, concept_id",
      );
