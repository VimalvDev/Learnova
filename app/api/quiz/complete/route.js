...
export async function POST(req) {
  try {
    ...
    const masteryUpdates = Object.entries(byConceptId).map(
      async ([conceptId, conceptAnswers]) => {
        const conceptCorrect = conceptAnswers.filter((a) => a.isCorrect).length;
        const conceptTotal = conceptAnswers.length;
        const conceptAccuracy = Math.round(
          (conceptCorrect / conceptTotal) * 100,
        );

        // Calculate speed score (faster = higher, cap at 100)
        const avgTime =
          conceptAnswers.reduce((s, a) => s + (a.timeTaken ?? 30), 0) /
          conceptAnswers.length;
        const speedScore = Math.min(
          100,
          Math.max(0, Math.round(100 - (avgTime / 60) * 100)),
        );

        // Error penalty — repeated wrong answers
        const errorPenalty = Math.min(
          25,
          conceptAnswers.filter((a) => !a.isCorrect).length * 8,
        );
        ...
        if (existing) {
          ...
          return {
            conceptId,
            before: Math.round(existing.score ?? 0),
            after: Math.round(newScore),
            delta: Math.round(newScore - (existing.score ?? 0)),
          };
        } else {
    ...
    const revisionInserts = masteryDeltas
      ...
      .map((m) => {
        ...
        const priority =
          m.after < 40 ? "critical" : m.after < 65 ? "high" : "medium";
        return {
          user_id: user.id,
